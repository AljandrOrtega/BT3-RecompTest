...
# ─── ISO prompt ─────────────────────────────────────────────────────────────────
if (-not $SkipSetup -and -not $LauncherOnly) {
    if (-not $Iso) {
        if ($ISO_DEFAULT) {
...
if (-not $LauncherOnly) {
    # Pre-configure the runner build dir
    ...
    if ($SkipSetup) {
        ...
        Remove-Item -Recurse -Force $STAGE -ErrorAction SilentlyContinue$setupArgs = @(
    ...
    $stageRunner = Get-ChildItem$STAGE -Filter "ps2EntryRunner.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
    if (-not $stageRunner) {
        if (Test-Path $existingBt3Runner) {
            Log "bt3-runner.exe already staged; skipping rename"
        } else {
            $buildRunner = Get-ChildItem$BUILD -Recurse -Filter "ps2EntryRunner.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($buildRunner) {
    ...
    Step "Modo LauncherOnly activo: omitiendo la compilación del Runner y setup.py"
    # Asegurarse de que el directorio $STAGE existe para copiar el ejecutable compilado
    New-Item -ItemType Directory -Force -Path $STAGE | Out-Null
}
