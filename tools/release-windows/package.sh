...
#   BT3-Recomp-x86_64.zip              portable folder + Launcher.bat wrapper
#   BT3-Recomp-x86_64.sha256          sha256 of the zip
#
# Windows ships as a plain portable folder: unzip anywhere and run Launcher.bat.
...
#   Launcher.bat        PATH/QT_PLUGIN_PATH wrapper
...
[[ -f "$STAGE/Launcher.exe" && -f "$STAGE/bt3-runner.exe" && -f "$STAGE/Launcher.bat" && -d "$STAGE/lib" ]] || {
    echo "ERROR: stage incomplete"; exit 2; }

# ---- portable tree ---------------------------------------------------------
TMP_TREE="$OUT_DIR/$TREE_NAME"
rm -rf "$TMP_TREE"; mkdir -p "$TMP_TREE"
cp -a "$STAGE"/Launcher.exe "$STAGE"/bt3-runner.exe "$STAGE"/Launcher.bat "$TMP_TREE"/
